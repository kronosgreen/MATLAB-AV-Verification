classdef (StrictDefaults)AVVerVisualization < matlab.System & matlab.system.mixin.CustomIcon
    ...
    properties(Nontunable, Logical)
        ...
        %HasLaneBoundaries Display lane boundaries
        HasLaneBoundaries = false;
    ...
    methods(Access=protected)
        function setupImpl(obj,varargin)
            ...
            if isBEPNeeded
                ...
                if obj.HasLaneBoundaries
                    bColors = get(obj.pBEP.Parent,'ColorOrder');
                    bColors = [1 0 0;0 1 0;bColors];
                    nColors = 1;
                    obj.pBoundaryPlotters = driving.birdsEyePlot.LaneBoundaryPlotter.empty;
                    lb = varargin{obj.pBoundaryInputIndex};
                    for jndx = 1:numel(lb.LaneBoundaries)
                        obj.pBoundaryPlotters(jndx) = laneBoundaryPlotter(obj.pBEP, ...
                            'DisplayName',['Lane Boundary' num2str(jndx)], 'Color', bColors(nColors,:));
                        nColors = nColors + 1;
                        if nColors > size(bColors,1)
                            nColors = 1;
                        end
                    end
                end
        ...
        function resetImpl(obj)
            ...
            % Plot coverage areas
            if obj.HasVision || obj.HasLaneBoundaries
        ...
        function s = saveObjectImpl(obj)
            ...
            if isLocked(obj)
                ...
                s.pBoundaryPlotters     = obj.pBoundaryPlotters;
                s.pLaneMarkingPlotter   = obj.pLaneMarkingPlotter;
            end
        end
        ...
        function loadObjectImpl(obj,s,wasLocked)
            if wasLocked
                ...
                obj.pBoundaryPlotters       = s.pBoundaryPlotters;
                ...
                s = rmfield(s,{'pFig','pBEP','pActorPlotter','pVisionPlotter', ...
                    ...
                    'pLaneMarkingPlotter','pBoundaryPlotters'});
            end
    ...
    methods(Access=private)
        ...
        function plotLaneBoundaries(obj,lbs)
            if isnumeric(lbs) && isscalar(lbs) && lbs == 0
                % Input is disconnected, so return
                return
            end

            if isscalar(lbs) && isfield(lbs,'IsValidTime') && ~lbs.IsValidTime
                % Input is disconnected, so return
                return
            end
            
            if isscalar(lbs) && isfield(lbs,'LaneBoundaries')
                lbs = lbs.LaneBoundaries;
            end
            
            for kndx = 1:length(lbs)
                obj.pBoundaryPlotters(kndx).clearData;
                lb = lbs(kndx);
                if isfield(lb,'Coordinates')
                    boundary = {lb.Coordinates};
                elseif isfield(lb,'CurvatureDerivative')
                    boundary = clothoidLaneBoundary;
                    boundary.Curvature = lb.Curvature;
                    boundary.CurvatureDerivative = lb.CurvatureDerivative;
                    boundary.CurveLength = lb.CurveLength;
                    boundary.HeadingAngle = lb.HeadingAngle;
                    boundary.LateralOffset = lb.LateralOffset;
                    boundary.BoundaryType = lb.BoundaryType;
                    boundary.Strength = lb.Strength;
                    boundary.Width = lb.Width;
                end
                plotLaneBoundary(obj.pBoundaryPlotters(kndx), boundary);
            end
        end
    end
    ...
    methods(Access=protected)
        ...
        function varargout = getInputNamesImpl(obj)
            ...
            if obj.HasLaneBoundaries
                varargout = {varargout{:} 'Lane Boundaries'}; %#ok<CCAT>
            end
            if obj.HasLaneMarkings
...
if numDets>0
    ...
    for m = 1:numDets
        posMeas = matlabshared.tracking.internal.fusion.parseDetectionForInitFcn...
			(dets.Detections(m), 'helperBirdsEyePlot', 'double');
        detPos(m,:) = posMeas;
    end
